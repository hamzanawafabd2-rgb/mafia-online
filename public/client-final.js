console.log("🎭 Mafia 60 FINAL — build 2026-09-19");
...
window.onload = function() {
  ...
  function setChatOpen(open) {
    ...
    if (dock) dock.classList.toggle("chat-open", !!open);
    if ($("btnMessages")) $("btnMessages").textContent = open ? "✕ إغلاق الرسائل" : "💬 الرسائل";
  ...
  $("btnCloseMessages").onclick = function() { setChatOpen(false); };
  ...
  function toggleMyMic() {
    ...
    if (me) { me.selfMuted = !t.enabled; me.micMuted = !t.enabled || !!me.hostMuted; }
  ...
  function isPlayerMuted(playerId) {
    ...
    if (playerId === myId) {
      ...
      var t = callStream.getAudioTracks()[0];
      return !!localForcedMuted || !t || !t.enabled;
    }
  ...
  socket.on("callSelfMuteState", function(d) {
    ...
    if (currentRoom && currentRoom.players) {
      var me = currentRoom.players.find(function(p){ return p.id === myId; });
      if (me) { me.selfMuted = !!d.selfMuted; me.hostMuted = !!d.hostMuted; me.micMuted = muted; }
    }
    updateMicButtonsForAll();
    refreshPlayerMicLayer();
  });
  ...
  socket.on("callPlayerMuteState", function(d) {
    ...
    if (currentRoom && currentRoom.players) {
      ...
      if (p) { p.micMuted = !!d.muted; p.hostMuted = !!d.hostMuted; p.selfMuted = !!d.selfMuted; }
    }
    updateMicButtonsForAll();
    refreshPlayerMicLayer();
  });
  socket.on("callHostMuteAck", function(d) {
    toast(d && d.muted ? "🔇 تم إسكات المجلس — لا يزال لكل لاعب كتمه الشخصي" : "🎙️ تم فتح المجلس — الكتم الشخصي محفوظ");
  });
  ...
  function renderPlayerMicLayer(room) {
    ...
    micLayerFrame = requestAnimationFrame(function() {
      ...
      room.players.forEach(function(p, i) {
        ...
        btn.style.left = Math.max(4, Math.min(window.innerWidth - 38, r.right + 4)) + "px";
        btn.style.top = Math.max(4, Math.min(window.innerHeight - 34, r.top + (r.height - 30) / 2)) + "px";
